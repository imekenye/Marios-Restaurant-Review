...
export default function Home() {
  // useContext state
  const { places, reviews, filtered } = useContext(PlacesContext);
  // check if is filtered
  const [isFiltered, setIsFiltered] = useState(false);

  // Image Url reference
  const getImageUrl = (photoRef) => {
    return `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photoRef}&key=${process.env.REACT_APP_GOOGLE_MAPS_API_KEY}`;
  };

  // on initial load
  useEffect(() => {
    if (filtered.length === 0 && isFiltered === false) {
      setIsFiltered(false);
    }
  }, [filtered.length, isFiltered]);
  return (
    <>
      <HeaderContainer setIsFiltered={setIsFiltered} />
      <MainContainer>
        <MapContainer />
        <RestaurantContainer>
          {filtered.length !== 0 && isFiltered === true ? (
            filtered &&
              ...
              >
                <div className="restaurant__image">
                  {restaurant.photos ? (
                    ...
                    >
                      {/* fallback svg image if no restaurant image */}
                      <svg
                        width="96"
                        height="96"
                        viewBox="0 0 96 96"
                ...
                <div className="restaurant__details">
                  <Link to={`/reviews/${restaurant.place_id}`}>
            ...
            <>
              ...
              <p>There's no results matching your criteria</p>
            </>
          ) : null}
          {isFiltered === false &&
            places &&
